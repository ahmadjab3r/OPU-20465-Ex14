...
bool handle_line(ASFile *as_file, char *str, bool with_label) {
    char *symbol_type, *symbol_name;
...
bool immediate_addressing(ASFile *as_file, char *line,
    ...
    //TODO write to file
    add_node("Immediate", line, number, &as_file->IC,true,
             as_file->lines);
    return true;
}
...
int check_addressing_type(ASFile *as_file, char *line,
    ...
    if (reg != -1) {
        if (type) {
            ...
            current_instruction->source_addressing =
                    REGISTER_ADDRESSING_SIGN;
        } else {
            current_instruction->destination_addressing = reg;
            current_instruction->destination_reg =
                    REGISTER_ADDRESSING_SIGN;
        }
        return 3;
    }
    ...
    while (fgets(line, LINE_LENGTH, input_file) != NULL) {
        ...
        if (strstr(line, LABEL_DEC)) {
            ...
            if (strstr(temp, STRING_DEC) || strstr(temp, DATA_DEC)) {
                handle_line(current_as_file, temp,true);
            }
            ...
            if (strstr(temp, STRING_DEC) || strstr(temp, DATA_DEC)) {
                handle_line(current_as_file, temp, false);
            }
        }
    }
}

...
{
  int index;
  struct Macro *new_macro, *current_macro;
   index = hash_function(macro_name, table->size);
...
int increase_table_size(struct MacroTable *table)
{
  ...
  struct Macro *temp = NULL, *pointer = NULL, *current_macro = NULL;
  struct Macro **new_macros;
  printf("function called");
  fflush(stdout);
  new_size = table->size * TABLE_MULTIPLIER;
  new_macros = malloc(new_size * sizeof(struct Macro));
  if (new_macros == NULL)
    {
      //TODO HANDLE ERROR
      return 0;
  ...
  for (i = 0; i < table->size; i++)
    {
      current_macro = table->macros[i];
      while (current_macro != NULL)
        {
           new_index = hash_function(current_macro->macro_name, new_size);
...
{
  int i;
  struct Macro *temp_macro = NULL, *current_macro = NULL;
